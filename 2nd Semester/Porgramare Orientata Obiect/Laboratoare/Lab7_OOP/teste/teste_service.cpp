...
void TesteService::test_filterByPret() {
    ...
    VectorDinamic<Produs> filtered = serv.filterByPret(1000, 2000);
...
void TesteService::test_filterByNume() {
    ...
    VectorDinamic<Produs> filtered = serv.filterByNume("Printer");
...
void TesteService::test_filterByProducator() {
    ...
    VectorDinamic<Produs> filtered = serv.filterByProducator("Oculus");
    assert(filtered.size() == 2);
...
void TesteService::test_sortByNume() {
    ...
    try {
        VectorDinamic<Produs> sorted = serv.sortByNume(true);
        assert(false);
    ...
    try {
        VectorDinamic<Produs> sorted = serv.sortByNume(false);
        assert(false);
    } catch (const RepoException& ex) {
        assert(true);
    }
...
void TesteService::test_sortByPret() {
    ...
    try {
        VectorDinamic<Produs> sorted = serv.sortByPret(true);
        assert(false);
    } catch (const RepoException& ex) {
        assert(true);
    }
    ...
    try {
        VectorDinamic<Produs> sorted = serv.sortByPret(false);
        assert(false);
    } catch (const RepoException& ex) {
        assert(true);
    }
...
void TesteService::test_sortByNumeSiTip() {
    ...
    try {
        VectorDinamic<Produs> sorted = serv.sortByNumeSiTip(true);
        assert(false);
    } catch (const RepoException& ex) {
        assert(true);
    }
    ...
    try {
        VectorDinamic<Produs> sorted = serv.sortByNumeSiTip(false);
        assert(false);
    } catch (const RepoException& ex) {
        assert(true);
    }
