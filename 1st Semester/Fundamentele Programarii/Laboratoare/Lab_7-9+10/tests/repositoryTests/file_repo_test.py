...
class TestRepository(unittest.TestCase):
    ...
    def setUp(self):
        self.studenti = FileRepoStudenti("studentTest.txt")
        self.discipline = FileRepoDiscipline("disciplinaTest.txt")
        self.note = FileRepoNote("gradesTest.txt")
        self.studenti.reset()
    ...
    def test_sterge_student_recursiv(self):
        self.studenti.adauga_student(self.student1)
        self.studenti.sterge_student_recursiv(self.student1.get_id())
        assert self.student1 not in self.studenti.get_all_students()

        try:
            self.studenti.sterge_student_recursiv(99999)
            assert False
        except RemoveStudentError:
            pass
    ...
    def test_sterge_disciplina_recursiv(self):
        self.discipline.adauga_disciplina(self.disciplina1)
        self.discipline.sterge_disciplina_recursiv(self.disciplina1.get_id())
        assert self.disciplina1 not in self.discipline.get_all_disciplines()

        try:
            self.discipline.sterge_disciplina_recursiv(9)
            assert False
        except RemoveDisciplineError:
            pass
...
def run_tests_file_repository():
    suite = unittest.TestLoader().loadTestsFromTestCase(TestRepository)
    runner = unittest.TextTestRunner()
    result = runner.run(suite)
    if result.wasSuccessful():
        print("Testele din file repo au mers")
    else:
        print("Niste teste din file repo au crapat")